...
interface AdminAuctionsProps {
    ...
    onSaveAuction: () => void
    onCancelEdit: () => void
    onAuctionDataChange: (field: string, value: any) => void
    onBidChange: (bidId: string, amount: number) => void
    onSaveBids: () => void
}
            ...
            >
                <div className="flex items-center gap-4 mb-6">
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg transition-all duration-200 ${auctionTab === 'pending'
                    ...
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg transition-all duration-200 ${auctionTab === 'active'
                                ? 'bg-blue-600 text-white'
                                : 'bg-gray-700 text-white/70 hover:text-white'
                            }`}
                    >
            ...
            >
                                ...
                                <tr key={auction.id} className="border-b border-gray-700 hover:bg-gray-800/50">
                                    ...
                                    <td className="py-3 px-4">
                                        <div className="text-white">
                                            ...
                                            {auction.buyNowPrice && (
                                                <p className="text-white/60 text-sm">Kup teraz: {auction.buyNowPrice.toFixed(2)} zł</p>
                                            )}
                                        </div>
                                    </td>
                                    <td className="py-3 px-4">
                ...
                <div className="flex items-center justify-between mt-6">
                    <p className="text-white/70 text-sm">
                        Strona {currentPage} z {auctionTab === 'pending' ? totalPages : activeTotalPages} ({currentTotal} aukcji)
                    </p>
                    <div className="flex items-center gap-2">
                        <button
                            onClick={() => {
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <div className="space-y-2 text-white/80">
                                    <p><span className="font-medium">Tytuł:</span> {selectedAuction.title}</p>
                                    <p><span className="font-medium">Opis:</span> {selectedAuction.description}</p>
                                    <p><span className="font-medium">Kategoria:</span> {selectedAuction.category}</p>
                                    <p><span className="font-medium">Cena startowa:</span> {selectedAuction.startingPrice.toFixed(2)} zł</p>
                                    <p><span className="font-medium">Aktualna cena:</span> {selectedAuction.currentPrice.toFixed(2)} zł</p>
                                    {selectedAuction.buyNowPrice && (
                                        <p><span className="font-medium">Kup teraz:</span> {selectedAuction.buyNowPrice.toFixed(2)} zł</p>
                                    )}
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="space-y-2 max-h-60 overflow-y-auto">
                                    {auctionBidders.map((bid) => (
                                        <div key={bid.id} className="flex items-center justify-between p-2 bg-gray-700 rounded">
                                            <div>
