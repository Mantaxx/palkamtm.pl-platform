...
export default function AdminDashboard() {
  ...
  const [editForm, setEditForm] = useState<{ firstName: string; lastName: string; role: string; isActive: boolean }>({ firstName: '', lastName: '', role: 'USER', isActive: true })
  ...
  const [referencesStatus, setReferencesStatus] = useState<'pending' | 'approved' | 'all'>('pending')
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        if (a) { setAuctions(a.auctions); setAuctionsTotal(a.total) }
        if (aa) { setActiveAuctions(aa.auctions); setActiveAuctionsTotal(aa.total) }
        if (r) { setReferences(r.references); setReferencesTotal(r.pagination?.total || 0) }
        if (m) { setMeetings(m.meetings); setMeetingsTotal(m.pagination?.total || 0) }
      } catch { }
    }
    fetchAll()
  ...
  const refreshAuctionLists = useCallback(async (silent = true) => {
    try {
      ...
      const refreshedPending = await fetch(`/api/admin/auctions/pending?page=${auctionsPage}&limit=${auctionsPageSize}`).then(r => r.json())
      setAuctions(refreshedPending.auctions); setAuctionsTotal(refreshedPending.total)

      // Odśwież aktywne aukcje
      const refreshedActive = await fetch(`/api/admin/auctions/active?page=${activeAuctionsPage}&limit=${activeAuctionsPageSize}`).then(r => r.json())
      setActiveAuctions(refreshedActive.auctions); setActiveAuctionsTotal(refreshedActive.total)
  ...
  }, [auctionsPage, auctionsPageSize, activeAuctionsPage, activeAuctionsPageSize, refreshAuctionLists])
  ...
  const saveBidEdit = async () => {
    ...
    setSavingBid(true)
    try {
      const response = await fetch(`/api/admin/auctions/${selectedAuction.id}/bids/${editingBid.id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedReferences = references.map(ref =>
          ref.id === referenceId ? { ...ref, isApproved } : ref
        ).filter(ref => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedMeetings = meetings.map(meeting =>
          meeting.id === meetingId ? { ...meeting, isApproved } : meeting
        ).filter(meeting => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 text-white">
        ...
        {activeTab === 'auctions' && (
          ...
          >
            <div className="bg-gray-800 border-2 border-white rounded-lg shadow-sm p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex bg-gray-700 rounded-lg p-1">
                  <button
                    ...
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${auctionTab === 'pending'
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${auctionTab === 'active'
                      ? 'bg-white text-gray-900'
                      : 'text-white hover:text-gray-300'
                      }`}
                  >
              ...
              {auctionTab === 'pending' && (
                <div>
                  <h3 className="text-lg font-medium text-white mb-4">Oczekujące aukcje do zatwierdzenia</h3>
                  ...
                  <div className="space-y-4">
                    {auctions.length === 0 ? (
                      ...
                      auctions.map((auction) => (
                        <div key={auction.id} className="bg-gray-700 rounded-lg p-4 border border-gray-600">
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-4 text-sm text-white/60">
                                ...
                                <span>Sprzedawca: {auction.seller.firstName} {auction.seller.lastName}</span>
                                <span>Data utworzenia: {new Date(auction.createdAt).toLocaleDateString()}</span>
                              </div>
                  ...
                  {auctionsTotal > 0 && (
                    <div className="mt-6 flex items-center justify-between">
                      <div className="flex items-center gap-2 text-sm text-white/70">
                        <span>Strona {auctionsPage} z {Math.max(1, Math.ceil(auctionsTotal / auctionsPageSize))}</span>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <select
                          ...
                          onChange={(e) => { setAuctionsPage(1); setAuctionsPageSize(parseInt(e.target.value, 10)) }}
              ...
              {auctionTab === 'active' && (
                <div>
                        ...
                        <div key={auction.id} className="bg-gray-700 rounded-lg p-4 border border-gray-600">
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-4 text-sm text-white/60">
                                <span>Start: {new Date(auction.startTime).toLocaleDateString()}</span>
                                <span>Koniec: {new Date(auction.endTime).toLocaleDateString()}</span>
                                {auction.bids && auction.bids.length > 0 && (
                                  <span>Najwyższa oferta: {auction.bids[0].bidder.firstName} {auction.bids[0].bidder.lastName} - {auction.bids[0].amount} zł</span>
                                )}
                              </div>
                              {auction.assets && auction.assets.length > 0 && (
                                <div className="mt-2">
                  ...
                  {activeAuctionsTotal > 0 && (
                    <div className="mt-6 flex items-center justify-between">
                      <div className="flex items-center gap-2 text-sm text-white/70">
                        <span>Strona {activeAuctionsPage} z {Math.max(1, Math.ceil(activeAuctionsTotal / activeAuctionsPageSize))}</span>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          disabled={activeAuctionsPage >= Math.ceil(activeAuctionsTotal / activeAuctionsPageSize)}
                        ...
                        <select
                          ...
                          onChange={(e) => { setActiveAuctionsPage(1); setActiveAuctionsPageSize(parseInt(e.target.value, 10)) }}
                          className="ml-2 border border-white/20 rounded px-2 py-1 bg-gray-800 text-white"
              ...
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${referencesStatus === 'pending'
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${referencesStatus === 'approved'
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${referencesStatus === 'all'
                    ...
                    <div key={reference.id} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">Doświadczenie: {reference.experience}</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className="flex items-center">
                            {[...Array(5)].map((_, i) => (
                          ...
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${reference.isApproved
                      ...
                      {reference.achievements && reference.achievements.length > 0 && (
                        <div className="mb-3">
                          ...
                          <div className="space-y-1">
                            {reference.achievements.map((achievement: {
                              pigeon: string;
                              ringNumber: string;
                              results?: Array<{
                                place: string;
                              ...
                              <div key={index} className="text-sm text-gray-600">
                                <strong>{achievement.pigeon}</strong> ({achievement.ringNumber}) - {achievement.results?.map((result) =>
        ...
        {activeTab === 'meetings' && (
          ...
          >
            <div className="bg-white border-2 border-white rounded-lg shadow-sm p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold text-gray-900">Zarządzanie spotkaniami z hodowcami</h2>
                <div className="flex gap-2">
                  <button
                    onClick={() => setMeetingsStatus('pending')}
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${meetingsStatus === 'pending'
                      ? 'bg-yellow-100 text-yellow-800 border border-yellow-300'
                      : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                      }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${meetingsStatus === 'approved'
                      ? 'bg-green-100 text-green-800 border border-green-300'
                      : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                      }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${meetingsStatus === 'all'
                      ? 'bg-blue-100 text-blue-800 border border-blue-300'
                      : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                      }`}
                  >
                  ...
                  meetings.map((meeting) => (
                    <div key={meeting.id} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${meeting.isApproved
                          ? 'bg-green-100 text-green-800'
                          : 'bg-yellow-100 text-yellow-800'
                          }`}>
                      ...
                      {meeting.images && meeting.images.length > 0 && (
                        <div className="mb-3">
                          <h4 className="font-medium text-gray-900 mb-2">Zdjęcia ({meeting.images.length}):</h4>
                          <div className="grid grid-cols-4 gap-2">
                            ...
                            {meeting.images.length > 4 && (
                              <div className="aspect-square bg-gray-100 rounded-lg flex items-center justify-center">
                                <span className="text-sm text-gray-600">+{meeting.images.length - 4}</span>
                              </div>
                            )}
                          </div>
                        </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {auctionBidders.map((bidder, index) => (
                      <div key={bidder.id} className="bg-gray-700 rounded-lg p-4 border border-gray-600">
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="font-semibold text-white">
                            {bidder.firstName} {bidder.lastName}
                          </h4>
                          <div className="flex items-center gap-2">
                            <span className={`px-2 py-1 rounded text-xs ${index === 0 ? 'bg-green-600 text-white' : 'bg-gray-600 text-gray-300'
                              }`}>
                            ...
                            <button
                              onClick={() => blockUser(bidder.id, `${bidder.firstName} ${bidder.lastName}`)}
                          ...
                          {editingBid && editingBid.id === bidder.id ? (
                            <div className="flex items-center gap-2">
                              ...
                              <input
                                ...
                                onChange={(e) => setEditingBid({ ...editingBid, amount: parseFloat(e.target.value) || 0 })}
                            ...
                            <p>Oferta: <span className="font-semibold text-green-400">{bidder.amount} zł</span></p>
                          )}
                          <p>Data: {new Date(bidder.createdAt).toLocaleString()}</p>
                          {bidder.totalBids && (
                            <p>Łączne licytacje: {bidder.totalBids}</p>
                          )}
                          {bidder.rating && (
                            <p>Ocena: {bidder.rating}/5 ⭐</p>
                          )}
                        </div>
                      </div>
                    ))}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditingAuctionData({ ...editingAuctionData, title: e.target.value })}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditingAuctionData({ ...editingAuctionData, currentPrice: e.target.value })}
                    className="w-full bg-gray-700 text-white px-3 py-2 rounded border border-gray-600 focus:border-blue-500"
                    min="0"
                    step="0.01"
                    placeholder="0.00"
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setEditingAuctionData({ ...editingAuctionData, status: e.target.value })}
                    className="w-full bg-gray-700 text-white px-3 py-2 rounded border border-gray-600 focus:border-blue-500"
