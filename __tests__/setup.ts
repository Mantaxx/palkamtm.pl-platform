...
vi.mock('@/lib/error-handling', () => ({
    AppErrors: {
        ...
        unauthorized: vi.fn(() => ({ message: 'Brak autoryzacji', type: 'AUTHENTICATION_ERROR', statusCode: 401 })),
        forbidden: vi.fn(() => ({ message: 'Brak uprawnień', type: 'AUTHORIZATION_ERROR', statusCode: 403 })),
        notFound: vi.fn((resource) => ({ message: `${resource} nie został znaleziony`, type: 'NOT_FOUND_ERROR', statusCode: 404 })),
        conflict: vi.fn((message) => ({ message, type: 'CONFLICT_ERROR', statusCode: 409 })),
        rateLimit: vi.fn(() => ({ message: 'Zbyt wiele żądań', type: 'RATE_LIMIT_ERROR', statusCode: 429 })),
        csrf: vi.fn(() => ({ message: 'Nieprawidłowy CSRF token', type: 'CSRF_ERROR', statusCode: 403 })),
        database: vi.fn((message) => ({ message, type: 'DATABASE_ERROR', statusCode: 500 })),
        externalService: vi.fn((service, message) => ({ message: message || `Błąd serwisu ${service}`, type: 'EXTERNAL_SERVICE_ERROR', statusCode: 502 })),
...
vi.mock('@/lib/api-middleware', () => ({
    ...
    createPaginatedResponse: vi.fn((data, page, limit, total) => ({ data, pagination: { page, limit, total } })),
...
vi.mock('@/lib/session-validation', () => ({
    ...
    validateAdminSession: vi.fn(() => ({ isValid: true, session: { user: { id: 'test-admin', role: 'ADMIN' } } })),
}))
