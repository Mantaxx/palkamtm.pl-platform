...
export async function GET(request: NextRequest) {
    try {
        ...
        const formattedConversations = conversations.map(conv => {
            ...
            return {
                ...
                lastMessage: lastMessage ? {
                    ...
                    senderName: `${lastMessage.sender.firstName || ''} ${lastMessage.sender.lastName || ''}`.trim() || 'Użytkownik',
        ...
        return NextResponse.json(
            { error: 'Nie udało się pobrać konwersacji' },
            { status: 500 }
        )
    }
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!recipient.isActive) {
            return NextResponse.json(
                { error: 'Odbiorca nie jest aktywny' },
                { status: 400 }
            )
        }
        ...
        return NextResponse.json({
            message: {
                ...
                senderName: `${message.sender.firstName || ''} ${message.sender.lastName || ''}`.trim() || 'Użytkownik',
