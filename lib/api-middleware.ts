...
export function createApiMiddleware(options: ApiMiddlewareOptions = {}) {
    ...
    return function wrapHandler(
        ...
        if (enableLogging) {
            wrappedHandler = withLogging(wrappedHandler) as (request: NextRequest) => Promise<NextResponse>
        }
        wrappedHandler = withErrorHandling(wrappedHandler) as (request: NextRequest) => Promise<NextResponse>

        // Cache - przed error handling
        if (enableCache) {
            wrappedHandler = withCache(wrappedHandler, cacheOptions) as (request: NextRequest) => Promise<NextResponse>
        }

        // Sanitization - przed cache
        if (enableSanitization) {
            wrappedHandler = withSanitization(wrappedHandler) as (request: NextRequest) => Promise<NextResponse>
        }
        ...
        // Auth - na początku
        if (needsAuth) {
            wrappedHandler = withAuth(wrappedHandler, requireAdmin) as (request: NextRequest) => Promise<NextResponse>
        }

        // Rate limiting - na samym początku
        if (enableRateLimit) {
            wrappedHandler = withRateLimit(wrappedHandler) as (request: NextRequest) => Promise<NextResponse>
        }
...
export function createApiRoute(
    ...
    const middlewareOptions = typeof config === 'string'
 ...
 */
export function logBusinessEventHelper(
    event: string,
    context?: Record<string, any>
) {
